...
A_DICT = {"key1": "val1", "key2": ["val2", "val3"]}
AN_EXPECTED_DICT = {"key1": ["val1"], "key2": ["val2", "val3"]}
...
def app():
    ...
    def echo(path):
        ...
        if path == "location":
            # Set to False by default in werkzeug 2.1
            # https://werkzeug.palletsprojects.com/en/2.2.x/wrappers/#werkzeug.wrappers.Response.autocorrect_location_header
...
def test_it_forwards_base_url(app):
    ...
    res = session.get(
        "https://awesome-app/location", headers={"Location": "/hello_world"}
    )
